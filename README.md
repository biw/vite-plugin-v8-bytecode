...
Ship Node.js and Electron applications without distributing their readable
JavaScript bundles. The plugin compiles Vite's CommonJS output to V8 cached
data, injects the loader needed to run it, and removes the original JavaScript
by default.
...
type CommonBytecodeOptions = {
  ...
  /**
   * Exact string literals to obfuscate with String.fromCharCode.
   *
   * This only prevents eligible literals from appearing verbatim in the
   * generated bytecode. It is reversible and must not be used for secrets,
   * credentials, or other security-sensitive values.
   * @default []
   */
