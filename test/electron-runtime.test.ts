...
describe.skipIf(!hasElectronDisplay)("Electron bytecode runtime", () => {
  it("compiles a batch in a real Electron main process", async () => {
    const bytecode = await compileToBytecodeBatchForRuntime([source, "module.exports = 42;"], {
      runtime: "electron",
    });
  ...
  it("runs Vite-produced bytecode through the generated loader in Electron", async () => {
    const applicationDirectory = await mkdtemp(path.join(tmpdir(), "vite-bytecode-electron-main-"));
...
`,
      ...
      expect(await readdir(outputDirectory)).toEqual(
        expect.arrayContaining(["bytecode-loader.cjs", "main.cjs", "main.cjsc"]),
      );
  ...
  it("resolves Electron from a supplied Vite project root", async () => {
    ...
    try {
      const electronDirectory = path.join(applicationDirectory, "node_modules", "electron");
      ...
      expect(resolveElectronPath(undefined, applicationDirectory)).toBe(expectedElectronPath);
    } finally {
      await rm(applicationDirectory, { force: true, recursive: true });
    }
  });
...
describe("Node runtime selection", () => {
  ...
  it("compiles only the CommonJS side of a mixed-output build", async () => {
    const applicationDirectory = await mkdtemp(path.join(tmpdir(), "vite-bytecode-mixed-runtime-"));
    const outputDirectory = path.join(applicationDirectory, "dist");
